...
pub fn args() -> Vec<Arg> {
    vec![
        ...
        // Output options
        Arg::new("tui-output")
            .help("Enable TUI progress display instead of plain text output (experimental)")
            .long("tui")
            .action(ArgAction::SetTrue),
    ]
}
...
pub fn exec(args: &ArgMatches) -> Result<()> {
    let use_tui = args.get_one::<bool>("tui-output").copied().unwrap_or(false)
        && std::io::stdout().is_terminal();

    let mut progress: Box<dyn ExportProgress> = if use_tui {
